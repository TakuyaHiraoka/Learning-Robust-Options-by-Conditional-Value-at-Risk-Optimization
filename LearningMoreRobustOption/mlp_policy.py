...
class MlpPolicy(object):
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True, num_options=2, dc=0):
        ...
        self.vpred = dense3D2(last_out, 1, "vffinal", option, num_options=num_options,
        ...
        self.op_pi = tf.nn.softmax(
            U.dense(tf.stop_gradient(last_out), num_options, "OPfc%i" % (i + 1), weight_init=U.normc_initializer(1.0)))
    ...
    def get_term_adv(self, ob, curr_opt):
        ...
        return (vals[curr_opt[0]] - np.sum((op_prob * vals), axis=0) + self.dc), (
        vals[curr_opt[0]] - np.sum((op_prob * vals), axis=0))
